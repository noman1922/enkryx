        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-20 grid grid-cols-1 gap-12 sm:grid-cols-2 lg:grid-cols-3">
            {data.projects.map((project, index) => (
              <div
                key={project.id}
                className={`group relative flex flex-col overflow-hidden rounded-3xl bg-gray-50 opacity-0 transition-all hover:bg-white hover:shadow-xl hover:ring-1 hover:ring-gray-100 animate-fade-in-up stagger-${(index % 3) + 1}`}
              >
                <a
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-8">
                    <h3 className="text-2xl font-bold text-black transition-colors group-hover:text-teal">{project.title}</h3>
                    <p className="mt-4 text-base leading-relaxed text-gray-600">{project.summary}</p>
                    <p className="mt-6 line-clamp-3 text-sm text-gray-500">{project.details}</p>
